...
const ProjectRecent = () => {
  ...
  const handleFavourite = (e, type, id) => {
    ...
    if (!user) {
      ...
      if (userProfile?.project_favorites?.some( p => p.id == id )){
  ...
  const checkFav = ({ id }) => {
    const isFavourite = userProfile?.project_viewed?.find(
        ...
        <div className="container py-5">
          <div className="row my-4">
            <div className="col-md-6 text-start">
              <h1 className="fw-bolder fa-3xl">
                ...
                {userProfile?.project_viewed &&
                  userProfile?.project_viewed?.length}
                )
              </h1>
            </div>
          ...
          <div className="row ">
            {loading ? (
              ...
              (userProfile?.project_viewed || []).slice().reverse().map((project) => (
                <div className="col-md-3 mb-5" key={project.id}>
                  <ProjectDataCard
                    ...
                    {...project}
                    isFavourite={userProfile?.project_favorites?.some( p => p.id == project.id )}
